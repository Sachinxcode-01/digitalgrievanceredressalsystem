...
export const AdminDashboard = ({ sessionUser, userProfile, onLogout }) => {
  ...
  const handleComposeBroadcast = async () => {
    ...
    setIsComposing(true);
    try {
      const res = await apiClient.post('/ai/compose-broadcast', { intent: broadcastIntent });
      const data = res.data;
      ...
      toast.error("AI composition failed: " + (err.response?.data?.error || err.message));
    } finally {
  ...
  const fetchAiSuggestion = async (ticket) => {
    ...
    try {
      const res = await apiClient.post('/ai/suggest', { ticket });
  ...
  const fetchSpecialistBriefing = async (ticket, dept) => {
    ...
    setIsBriefing(true);
    try {
      const res = await apiClient.post('/ai/elevate', { ticket, department: dept });
      setSpecialistBriefing(res.data?.briefing || '');
    } catch (err) {
