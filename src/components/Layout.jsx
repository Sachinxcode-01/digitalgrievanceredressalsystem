...
import { LogOut, LayoutDashboard, Ticket, Users, Bell, Search } from 'lucide-react';
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          {user.role === 'admin' ? (
            <>
              ...
              <NavItem icon={<Ticket size={20} />} label="All Tickets" />
            </>
          ) : (
            <>
              <NavItem icon={<Ticket size={20} />} label="My Grievances" />
            </>
          )}
        </nav>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-white/10">
              <div className="text-right">
                <p className="text-sm font-bold text-white leading-none mb-1">{user.user_metadata?.full_name || user.email.split('@')[0]}</p>
